...
public function select($includes = array(), $excludes = array())
	  ...
	  public function save($collection = "", $insert = array())
        {
                ...
                try
                {
                        $this->db->{$collection}->save($insert, array($this->query_safety      => TRUE));
                        if (isset($insert['_id']))
                        {
                                return ($insert['_id']);
                        }
	...
	*/
	...
	public function update_push($collection = "", $data = array(), $options = array())
	{
		if (empty($collection))
		{
			show_error("No Mongo collection selected to update", 500);
		...
		{
			$options = array_merge($options, array($this->query_safety => TRUE, 'multiple' => FALSE));
			$this->db->{$collection}->update($this->wheres, array('$push' => $data), $options);
			$this->_clear();
			return (TRUE);
		}
		catch (MongoCursorException $e)
	...
	public function update_pull($collection = "", $data = array(), $options = array())
	{
		if (empty($collection))
		{
			show_error("No Mongo collection selected to update", 500);
		...
		{
			$options = array_merge($options, array($this->query_safety => TRUE, 'multiple' => FALSE));
			$this->db->{$collection}->update($this->wheres, array('$pull' => $data), $options);
			$this->_clear();
			return (TRUE);
		}
		catch (MongoCursorException $e)
	...
	*/
	...
	public function update_pushall($collection = "", $data = array(), $options = array())
	{
		if (empty($collection))
		{
			show_error("No Mongo collection selected to update", 500);
		...
		{
			$options = array_merge($options, array($this->query_safety => TRUE, 'multiple' => FALSE));
			$this->db->{$collection}->update($this->wheres, array('$pushAll' => $data), $options);
			$this->_clear();
			return (TRUE);
		}
		catch (MongoCursorException $e)
	...
	public function update_push_all($collection = "", $data = array())
	{
		if (empty($collection))
		{
			show_error("No Mongo collection selected to update", 500);
		...
		try
		{
			$this->db->{$collection}->update($this->wheres, array('$push' => $data), array($this->query_safety => TRUE, 'multiple' => TRUE));
			$this->_clear();
			return (TRUE);
		}
		catch (MongoCursorException $e)
